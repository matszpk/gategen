    ...
    <isize as TryFrom<T>>::Error: Debug,
{
    ...
    /// Generates circuit for given self (as outputs) variable.
    ///
    /// `iter` is list of inputs that should be mapped into circuit inputs.
    ...
    where
        ...
        I: Iterator<Item = BoolExprNode<T>>,
    {
        use crate::boolexpr::input_map_to_input_list_with_map;
        let (circuit, input_map) = self.to_circuit();
    ...
    where
        ...
        <usize as TryFrom<<T as VarLit>::Unsigned>>::Error: Debug,
    {
        DynIntExprNode::from_boolexprs(BoolExprNode::from_circuit(circuit, inputs))
    }
}
