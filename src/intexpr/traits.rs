...
pub struct MyBitIter<I, B> {
    iter: I,
    b: B,
}

impl<I, B> Iterator for MyBitIter<I, B>
where
    B: BitVal + Copy + Sized,
...
impl<I, B> DoubleEndedIterator for MyBitIter<I, B>
where
    B: BitVal + Copy + Sized,
...
pub trait BitVal {
    ...
    /// Get iterator.
    fn iter(self) -> MyBitIter<std::ops::Range<usize>, Self>
    where
        Self: Sized + Copy,
    {
        MyBitIter {
...
pub trait ExtraOps {
    ...
    /// Returns if and only if `self == 2^k` for some `k`.
    fn is_power_of_two(self) -> Self::BoolOutput;
    /// Reverses order of bits in the integer.
